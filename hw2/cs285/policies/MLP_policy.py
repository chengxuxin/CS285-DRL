...
class MLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        dist = self(torch.from_numpy(observation).type(torch.float))
        action = dist.rsample().detach().numpy()
    ...
    def forward(self, observation: torch.FloatTensor):
        ...
        # normal_dist = torch.distributions.normal.Normal(self.mean_net(observation), torch.exp(self.logstd))
        mul_normal_dist = ptu.multivariate_normal_diag(self.mean_net(observation), torch.exp(self.logstd))
        return mul_normal_dist
